...
FUAgoraDemo 是 Faceunity 的面部跟踪和虚拟道具功能在声网 OpenLive_iOS 及 OpenVideoCall_iOS 中的集成。声网原版文档可以参考[这里](http://docs.agora.io/cn/)。
...
然后对 faceunity 环境进行初始化。其中 `g_auth_package` 为密钥数组，必须传入密钥后，SDK才能正常工作。
...
在 `YuvPreProcessorProtocol` 的 `func onFrameAvailable(....)` 回调接口中调用道具绘制函数进行绘制:
